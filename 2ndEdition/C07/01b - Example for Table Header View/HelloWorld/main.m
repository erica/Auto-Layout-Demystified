...
- (void) updateConstraints
{
    ...
    for (NSLayoutConstraint *constraint in iv.referencingConstraintsInSuperviews)
        [constraint remove];
...
- (instancetype) initWithFrame:(CGRect)frame
{
    ...
    iv = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Icon.png"]];
    [self.contentView addSubview:iv];
    PREPCONSTRAINTS(iv);
    
    self.contentView.backgroundColor = ORANGE_COLOR;
...
- (void) loadView
{
    ...
    searchString = @"Charlie Daniels"; // The devil had tea and crumpets in Georgia
    
    self.tableView.tableHeaderView = [[CustomHeader alloc] initWithFrame:CGRectMake(0,0,0,61)];
}
@end
